...
exports.searchPost = async (req, res) => {
    ...
    if(!title.trim()) return res.status(401).json({error: '검색어가 없습니다.'})
      
    const posts = await Post.find({ title: { $regex: title, $options: "i"} })
    //regex 정규식,  옵션 i 는 대소문자 구분 하지 않게
...
exports.getRelatedPosts = async (req, res) => {
    const { postId } = req.params;
    if(!isValidObjectId(postId)) return res.status(401).json({error: '에러가 발생했습니다.'});

    const post = await Post.findById(postId)
    if(!post)  return res.status(401).json({error: '에러가 발생했습니다.'});
...
exports.uploadImage = async(req, res) => {
    const { file } = req
    if(!file) return res.status(401).json({error: '에러가 발생했습니다.'});
